...
static JSON_Malloc_Function parser_malloc = malloc;
static JSON_Free_Function parser_free = free;
...
static char * parser_strndup(const char *string, size_t n);
static char * parser_strdup(const char *string);
...
/* Various */
static char * parser_strndup(const char *string, size_t n) {
    char *output_string = (char*)parser_malloc(n + 1);
    if (!output_string) {
        return NULL;
    }
    output_string[n] = '\0';
...
static char * parser_strdup(const char *string) {
    return parser_strndup(string, strlen(string));
}
...
static char * read_file(const char * filename) {
    ...
    file_size = pos;
    rewind(fp);
    file_contents = (char*)parser_malloc(sizeof(char) * (file_size + 1));
    if (!file_contents) {
        fclose(fp);
        return NULL;
    }
...
/* JSON Object */
static JSON_Object * json_object_init(void) {
    JSON_Object *new_obj = (JSON_Object*)parser_malloc(sizeof(JSON_Object));
    if (!new_obj) {
        return NULL;
    }
...
static JSON_Status json_object_add(JSON_Object *object, const char *name, JSON_Value *value) {
    ...
    index = object->count;
    object->names[index] = parser_strdup(name);
...
static JSON_Status json_object_resize(JSON_Object *object, size_t new_capacity) {
    ...
    temp_names = (char**)parser_malloc(new_capacity * sizeof(char*));
    if (temp_names == NULL) {
        return JSONFailure;
    }

    temp_values = (JSON_Value**)parser_malloc(new_capacity * sizeof(JSON_Value*));
    if (temp_values == NULL) {
        parser_free(temp_names);
        return JSONFailure;
    }
    ...
    parser_free(object->names);
    parser_free(object->values);
...
static void json_object_free(JSON_Object *object) {
    while(object->count--) {
        parser_free(object->names[object->count]);
...
/* JSON Array */
static JSON_Array * json_array_init(void) {
    JSON_Array *new_array = (JSON_Array*)parser_malloc(sizeof(JSON_Array));
    if (!new_array) {
        return NULL;
    }
...
static JSON_Status json_array_resize(JSON_Array *array, size_t new_capacity) {
    ...
    new_items = (JSON_Value**)parser_malloc(new_capacity * sizeof(JSON_Value*));
    if (new_items == NULL) {
        return JSONFailure;
    }
...
/* JSON Value */
static JSON_Value * json_value_init_string_no_copy(char *string) {
    JSON_Value *new_value = (JSON_Value*)parser_malloc(sizeof(JSON_Value));
    if (!new_value) {
        return NULL;
    }
...
static char* process_string(const char *input, size_t len) {
    ...
    size_t final_size = 0;
    char *output = (char*)parser_malloc(initial_size);
    ...
    *output_ptr = '\0';
    ...
    resized_output = (char*)parser_malloc(final_size);
...
JSON_Value * json_parse_string_with_comments(const char *string) {
    ...
    string_mutable_copy = parser_strdup(string);
    ...
    parser_free(string_mutable_copy);
    return result;
}
    ...
    switch (json_value_get_type(value)) {
        ...
        case JSONString:
            if (value->value.string) {
                parser_free(value->value.string);
            }
            break;
        case JSONArray:
...
JSON_Value * json_value_init_string(const char *string) {
    ...
    copy = parser_strndup(string, string_len);
    if (copy == NULL) {
        return NULL;
    }
...
JSON_Value * json_value_deep_copy(const JSON_Value *value) {
    ...
    switch (json_value_get_type(value)) {
        ...
        case JSONString:
            ...
            temp_string_copy = parser_strdup(temp_string);
            if (temp_string_copy == NULL) {
                return NULL;
            }
            return_value = json_value_init_string_no_copy(temp_string_copy);
            if (return_value == NULL) {
                parser_free(temp_string_copy);
            }
            return return_value;
        case JSONNull:
            return json_value_init_null();
...
char * json_serialize_to_string(const JSON_Value *value) {
    ...
    buf = (char*)parser_malloc(buf_size_bytes);
    if (buf == NULL) {
        return NULL;
    }
...
JSON_Status json_object_dotset_value(JSON_Object *object, const char *name, JSON_Value *value) {
    ...
    if (dot_pos == NULL) {
        ...
        current_name = parser_strndup(name, dot_pos - name);
        temp_obj = json_object_get_object(object, current_name);
        if (temp_obj == NULL) {
            new_value = json_value_init_object();
            if (new_value == NULL) {
                parser_free(current_name);
                return JSONFailure;
            }
...
void json_set_allocation_functions(JSON_Malloc_Function malloc_fun, JSON_Free_Function free_fun) {
    parser_malloc = malloc_fun;
    parser_free = free_fun;
}
